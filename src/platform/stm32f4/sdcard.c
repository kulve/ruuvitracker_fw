...
//For 2.8V
//#define SD_VOLTAGE_WINDOW_SD            ((uint32_t)0x80010000)
...
static void checkerror(char *s, int e)
{
  if (e)
    printf("%s returned %d\n", s, e);
}
...
SD_Error SD_Init(void)
{
  ...
  SD_LowLevel_Init();
  printf("SD_LowLevel_Init(): done\n");

  SDIO_DeInit();
  printf("SDIO_Deinit(): done\n");

  errorstatus = SD_PowerON();
  printf("SD_PowerON, returned %d\n", errorstatus);
  ...
  printf("SD_InitializeCards() returned %d\n", errorstatus);
  ...
  printf("SD_GetCardInfo() returned %d\n", errorstatus);
...
//      SDIO_CmdInitStructure.SDIO_Argument = 0x80000000;
